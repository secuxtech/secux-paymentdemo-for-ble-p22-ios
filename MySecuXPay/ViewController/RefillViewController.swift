...
class RefillViewController: BaseViewController {
    ...
    lazy var titleImg: UIImageView = {
        ...
        imageView.image = UIImage(named: "refill_icon")
    ...
    lazy var accountLabel : UILabel = {
        ...
        NSLayoutConstraint.activate([
            ...
            label.leftAnchor.constraint(equalTo: self.refillButton.leftAnchor, constant: 0)
    ...
    lazy var refillButton:  UIRoundedButtonWithGradientAndShadow = {
        ...
        btn.setTitle(NSLocalizedString("Refill", comment: ""), for: .normal)
        //btn.setBackgroundColor(UIColor(red: 0.8, green: 0.8, blue: 0.8, alpha: 1), for:.disabled)
        btn.addTarget(self, action: #selector(refillAction), for: .touchUpInside)
    ...
    @objc func refillAction(){
        ...
        DispatchQueue.global().async {
            ...
            if self.nonce.count == 0{
                (ret, info) = self.paymentMgr!.doRefill(devID: self.deviceID, devIDHash: self.deviceIDhash)
            }else{
                (ret, info) = self.paymentMgr!.doRefill(nonce:self.nonce, devID: self.deviceID, devIDHash: self.deviceIDhash)
            }
            ...
            if ret == .SecuXRequestNoToken || ret == .SecuXRequestUnauthorized{
                ...
                DispatchQueue.main.async {
                    ...
                    let vc = RefillResultViewController()
