    ...
    func setup(hasLeftIcon: Bool, hasRightTxt: Bool){
        ...
        if self.hasLeftIcon{
            NSLayoutConstraint.activate([
                ...
                self.theTextField.leftAnchor.constraint(equalTo: self.itemImage.rightAnchor, constant: 10),
                self.theTextField.topAnchor.constraint(equalTo: self.topAnchor),
                self.theTextField.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -3),
                //textfield.rightAnchor.constraint(equalTo: self.theTextLabel.leftAnchor, constant: -16),
                
                self.theTextField.widthAnchor.constraint(equalTo: self.widthAnchor, constant: -32-self.theTextLabel.intrinsicContentSize.width-15)
            ])
        }else if self.hasRightTxt{
            NSLayoutConstraint.activate([
            
                self.theTextField.leftAnchor.constraint(equalTo: self.leftAnchor, constant: 10),
                self.theTextField.topAnchor.constraint(equalTo: self.topAnchor),
                self.theTextField.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -3),
                //textfield.rightAnchor.constraint(equalTo: self.theTextLabel.leftAnchor, constant: -16)
                
                self.theTextField.widthAnchor.constraint(equalTo: self.widthAnchor, constant: -self.theTextLabel.intrinsicContentSize.width-15)
            ])
        }else{
            NSLayoutConstraint.activate([
                ...
                self.theTextField.widthAnchor.constraint(equalTo: self.widthAnchor, constant: -1)
            ])
        }
    }
